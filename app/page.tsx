...
function ModelStatus({
  ...
  state: ReturnType<typeof useStudio>["segState"];
  mock: boolean;
}) {
  if (mock) {
    return (
    ...
    return (
      <div className="rounded-2xl border-2 border-lilac/50 bg-lilac/15 px-3 py-2">
        <p className="text-[11px] font-bold text-ink">
          Waking up the cut-out brain… {Math.round(state.progress * 100)}%
        </p>
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 pb-16 pt-6 sm:px-6">
      <header className="relative mb-6 text-center">
        <span className="pointer-events-none absolute left-2 top-2 hidden text-3xl opacity-40 animate-float sm:block" style={{ ["--tilt" as string]: "-12deg" }} aria-hidden>
          🐾
        </span>
        <span className="pointer-events-none absolute right-3 top-6 hidden text-2xl opacity-40 animate-float sm:block" style={{ ["--tilt" as string]: "14deg" }} aria-hidden>
          ✂️
        </span>
        ...
        <p className="mx-auto mt-1 max-w-lg text-sm font-semibold text-ink-soft">
          Drop in your photos, we snip out the star of each one, and you get a printable sheet of
          die-cut stickers. Everything happens right here in your browser. 🐈
        </p>
      </header>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,370px)_minmax(0,1fr)]">
        <Card className="p-4 lg:col-start-1 lg:row-start-1">
          <SectionTitle icon="📸" hint={studio.items.length ? `${studio.items.length} photo${studio.items.length === 1 ? "" : "s"}` : undefined}>
            Your photos
          </SectionTitle>
